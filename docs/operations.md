...
(`ca_pending`), планирование волны одобрений при продлении сертификатов
и поэтапное обновление агентов на флоте. Про отказоустойчивость при
нескольких репликах сервера — отдельная страница, [Отказоустойчивость
(HA)](ha.md), читайте её тоже, если планируете горизонтальное
масштабирование.
...
- При ручном одобрении заявок ожидайте периодическую волну заявок на
  одобрение по мере того, как у устройств флота подходит к концу
  3-летний (по умолчанию) срок действия сертификата, а не равномерный
  редкий поток.
...
## Обновление агентов на флоте

Сервер умеет поэтапно раскатывать на флот новую версию агента, не
открывая для этого отдельный канал доставки, — агент узнаёт об
обновлении в рамках своего обычного цикла работы.

- Администратор задаёт минимальную требуемую версию агента.
- Раскатка постепенная: отдельно задаётся процент флота (0–100%),
  которому предлагается обновление, — можно сначала проверить его на
  части флота (canary), а затем расширять до полного охвата.
- Раскатку можно ограничить конкретным набором целевых платформ.
- Обновление доставляется не отдельным каналом, а подписанным манифестом
  в ответе на очередной checkin (`POST /api/v1/requests/{id}/checkin`,
  см. [REST API](api.md#enrollment-и-агент-agent)).
- Устройство, которому выдан манифест, помечается ожидающим обновления.
  Если оно не переходит на целевую версию дольше
  `ALATYR_AGENT_UPDATE_ALERT_MINUTES` (по умолчанию 30 минут, см.
  [Конфигурация](configuration.md)), фоновая сверка помечает его как
  застрявшее и заносит это в алерты.
- Список застрявших устройств отдаёт административный эндпоинт `GET
  /api/v1/settings/agent-update/stuck` (см. [REST API](api.md)).
- Минимальная версия, процент раскатки и набор платформ настраиваются на
  вкладке «Agent Update» в Admin UI (см. [Администрирование → Обзор
  разделов Admin UI](administration.md#обзор-разделов-admin-ui)).
