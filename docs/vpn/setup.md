...
На стенде, поднятом набором `deploy/` (`first-up.sh`), этот шаг уже сделан:
...
Отчёту нужно знать, чей сертификат искать. Команду запускает сам сотрудник,
в своём сеансе — хранилище `CurrentUser` у каждого своё:
...
#### Профиль целиком и куда его положить

Профиль клиента пишете вы; строку `cryptoapicert` берёте из отчёта. Пример,
которым поднят туннель на проверочном стенде:
...
cryptoapicert "THUMB:<отпечаток из alatyr-agent vpn>"
<ca>
... корень, которым подписан сертификат ВАШЕГО сервера доступа ...
</ca>
```

**Кладите профиль в `C:\Program Files\OpenVPN\config\`** — это делает
администратор (или политика раскатки). Оттуда OpenVPN GUI запускает профиль
любому пользователю. Профиль из пользовательского каталога
`%USERPROFILE%\OpenVPN\config\` непривилегированный сотрудник запустить не
сможет без членства в группе `OpenVPN Administrators` — подробности в
[«Нюансах и пределах»](caveats.md#профиль-из-пользовательского-каталога-требует-группы-openvpn-administrators).

Один и тот же профиль **не должен** лежать в обоих каталогах — см. про дубль
имени ниже.
