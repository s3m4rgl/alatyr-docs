...
5. **Агент на macOS.** Готовый `.pkg` из выпуска ставится вместе с файлом
   ...
   планируете выдавать `user_mtls` для входа в браузере, раздайте на Mac
   сотрудников корневой сертификат Alatyr как доверенный — через MDM либо
   вручную в «Связке ключей».
