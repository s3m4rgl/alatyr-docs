...
Keyholder API — отдельный неавторизованный (per-IP allowlist, не
per-role) HTTP-эндпоинт, который `sshd` на каждом хосте опрашивает через
`AuthorizedKeysCommand`, чтобы получить актуальный список публичных
SSH-ключей зарегистрированного пользователя из PostgreSQL.
...
SSH-ключи не выпускаются автоматически: `/enroll/ssh`, как и
`/enroll/user`, всегда оставляет заявку в `pending` — обязательное
ручное подтверждение. Каждое изменение состояния (одобрение, отзыв,
