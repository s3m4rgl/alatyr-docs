...
### Что именно доказывает эта подпись — и чего не доказывает

Стоит понимать точно, иначе легко переоценить защиту.

Сервер проверяет подпись **против открытого ключа, взятого из самого
присланного CSR**. Это доказывает, что заявку подписал тот, кто владеет
ключом, которым построен этот CSR — то есть связывает тело заявки с ключом
и закрывает две конкретные атаки: переклейку чужой аттестации на свою
заявку и повтор ранее перехваченной заявки (нонс одноразовый, окно по
времени узкое).

Чем эта подпись **не является — аутентификацией устройства.** Сама по себе
она не отвечает на вопрос «а тот ли это ключ, который мы ожидаем от этой
машины»: отправитель, сгенерировавший собственную пару ключей и собственный
CSR, подпишет заявку этим же ключом и проверку пройдёт.

Ответ на этот вопрос даёт **аппаратная аттестация**, а не подпись:

- **Windows и Linux** — `TPM2_Certify` привязывает ключ CSR к TPM, чей
  EK-сертификат подписан производителем. Здесь ступени `attestation_hardgate`
  и `strict` действительно поднимают планку до «ключ резидентен в
  проверяемом аппаратном модуле».
- **macOS** — такой привязки не существует до macOS 27, см.
  [Известные ограничения](limitations.md). Поэтому на macOS любая ступень
  source-auth оставляет барьером **ручное одобрение заявки**, а не саму
  подпись.
...
1. **Не выставляйте `/enroll*` в интернет.** Эндпоинты регистрации не
   требуют авторизации по устройству протокола, поэтому доступность их
   снаружи — самостоятельный риск. Ограничьте доступ корпоративной сетью
   или VPN. Это единственная мера, которая полностью снимает сценарий
   «заявку прислали произвольным HTTP-клиентом извне», и она не требует
   изменений в агентах.
2. **Не включайте автоодобрение для macOS.** Сервис-аккаунт с ролью
   `cert-auto-approver` не может авто-одобрить заявку с платформой Secure
   Enclave, пока не настроена corp-ownership-проверка — это поведение по
   умолчанию, и его не стоит ослаблять. Для macOS предпочтительно ручное
   одобрение.
3. **Настройте corp-ownership-проверку** (`sa_auto_approve_verifier`).
   Список разрешённых серийных номеров помогает только против выдуманных
   серийников; против настоящего серийника из вашего парка эффективен
   вариант `webhook` — запрос в вашу систему учёта или MDM с вопросом,
   действительно ли эта машина сейчас управляется и на связи.
4. **Проверяйте при одобрении, что заявка ожидаема.** Полезные признаки в
   ...
   несколько заявок на один серийный номер подряд. Каждое решение
   об одобрении попадает в Аудит с указанием, кто его принял.
5. **Включите `TPM_REJECT_SOFTWARE`** для парка, где аппаратный ключ
   обязателен — это отсекает заявки вообще без TPM и Secure Enclave, хотя
   и не отличает подлинный Secure Enclave от заявленного.

Для Windows и Linux дополнительно доступны ступени `attestation_hardgate`
и `strict` — там они дают настоящую аппаратную гарантию, а не только
...
Читают список сетей `cert-admin`, `cert-approver` и `cert-viewer`; создаёт,
отключает, восстанавливает и удаляет записи только `cert-admin`. Он же
задаёт имя RADIUS-сервера и переключает распространение профиля через
агента.

Сам раздел — что такое сеть в Alatyr, чем Wi-Fi отличается от проводной и
что происходит на устройстве после выдачи — [Wi-Fi и проводной
802.1X](wifi/index.md):

| Если вам нужно | Читайте |
|---|---|
| Завести сеть и раздать профиль | [Настройка сети](wifi/setup.md) |
| Задать имя RADIUS-сервера | [Имя RADIUS-сервера](wifi/server-name.md) |
| Отдать профиль MDM, GPO или config management | [Кто ставит профиль](wifi/setup.md#agent-profile-disabled) |
