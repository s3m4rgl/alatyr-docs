...
этих имён или смене корня PKI. Файлы, подложенные в том `tls_data` в обход
этого механизма, служба заменит при ближайшей проверке (не реже раза в
12 часов) — она не узнаёт в них действующий сертификат и считает, что
сертификата ещё нет.
