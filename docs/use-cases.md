...
играет карта, создаваемая агентом на самом устройстве. Пользователь входит по
PIN карты, а не по доменному паролю.
...
[реестре ключей](ssh/registry.md); список эндпоинтов — [REST API](api.md).
...
`ad_logon`, `ssh` и `k8s` несут в себе идентичность человека и всегда требуют
...
Подробнее — [Администрирование](administration.md). Цели выпуска независимы: можно
